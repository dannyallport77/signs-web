  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const errorMsg = data.error || `Failed to ${editingUser ? 'update' : 'create'} user`;
        console.error('User operation failed:', data);
        alert(errorMsg);
      }
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="text-sm text-gray-900 font-medium">Active Users</div>
          <div className="text-2xl font-bold text-green-600">
            {users.filter(u => u.active).length}
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="text-sm text-gray-900 font-medium">Admins</div>
